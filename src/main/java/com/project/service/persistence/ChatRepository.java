...
public interface ChatRepository extends JpaRepository<ChatEntity, UUID> {
    ...
    List<ChatEntity> findChatByUserId(@Param("userId") UUID userId);

    @Query("select c from ChatEntity c where c.isGroup=false and :user member of c.users and :reqUser member of c.users")
    ChatEntity findSingleChatByUserIds(@Param("user") UserEntity user, @Param("reqUser") UserEntity reqUser);
