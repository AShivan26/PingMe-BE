...
@Controller
@RequestMapping("/api")
public class UserDetailsApiController implements UserDetailsApi {

    private final UserDetailsApiDelegate delegate;

    public UserDetailsApiController(@Autowired(required = false) UserDetailsApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new UserDetailsApiDelegate() {});
    }

    @Override
    public UserDetailsApiDelegate getDelegate() {
        return delegate;
    }
