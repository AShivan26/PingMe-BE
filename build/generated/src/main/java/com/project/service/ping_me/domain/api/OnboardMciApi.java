...
@Validated
@Controller
@Tag(name = "OnboardMci", description = "the OnboardMci API")
public interface OnboardMciApi {

    default OnboardMciApiDelegate getDelegate() {
        return new OnboardMciApiDelegate() {};
    }
    ...
    @Operation(
        ...
        responses = {
            @ApiResponse(responseCode = "200", description = "Returns entry response object", content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = OnBoardMCIResponse.class))
            }),
            ...
            @ApiResponse(responseCode = "416", description = "HTTP errors may have one of the following error codes: - `REQUESTED_RANGE_NOT_SATISFIABLE` - Vehicle Selection not Allowed Currently. - `BOOKING_NOT_ELIGIBLE` - in case of non zen or next not eligible bookings"),
    ...
    @RequestMapping(
        ...
        value = "/{rentalActivityId}/onboarding-info",
        produces = { "application/json" }
    )
    default ResponseEntity<OnBoardMCIResponse> mCIOnboard(
        ...
        return getDelegate().mCIOnboard(rentalActivityId);
    }
